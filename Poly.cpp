...
void Poly::setCoef(int i,double value) {
  ...
  if (i< 0 || i> grau){
    std::cerr <<  "[ERRO] O Indice não pode ser maior que o grau ou menor que 0" << std::endl;
    return;
  } 
  if (value == 0 && i == grau && grau >0) {
    std::cerr << "[ERRO] valor nulo para o coeficiente de maior grau em um polinômio não nulo"<< std::endl;
    return;
  }
